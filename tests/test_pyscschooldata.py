...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = sc.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyscschooldata as sc
        assert hasattr(sc, 'fetch_enr')
        assert hasattr(sc, 'fetch_enr_multi')
        assert hasattr(sc, 'tidy_enr')
        assert hasattr(sc, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = sc.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = sc.fetch_enr(years['max_year'])
        # Total enrollment in SC varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # SC should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = sc.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = sc.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyscschooldata as sc
        years = get_test_years()
        df_single = sc.fetch_enr(years['max_year'])
        df_multi = sc.fetch_enr_multi([years['max_year']])
