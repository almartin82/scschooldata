# ==============================================================================
# LIVE Pipeline Tests for scschooldata
# ==============================================================================
#
...
test_that("South Carolina DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add South Carolina DOE URL and verify HTTP 200")
})
...
test_that("Can download South Carolina enrollment data file", {
  skip_if_offline()
...
test_that("Can parse South Carolina enrollment file", {
  skip_if_offline()
...
test_that("scschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- scschooldata::get_available_years()
  ...
  tryCatch({
    raw <- scschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- scschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- scschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- scschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- scschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- scschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- scschooldata:::get_cache_path(2024, "enrollment")
