...
warnings - \[ \] `pytest tests/test_pyscschooldata.py` — all tests
pass - \[ \]
...
`lep`, `econ_disadv`, `male`, `female`

**NOT in tidy enrollment:** SC data comes from two separate files (grade
file and demographic file). Demographics are merged by school/district
ID. `lep` and `special_ed` may be NA in the demographic file. SC also
has an `EE` (Early Education) grade level in some years.
...
**Common trap:** SC includes `EE` (Early Education, mapped from
`grade_ee`) in some years, which is separate from `PK`. The K12
aggregate excludes both `PK` and `EE`.

### entity flags

`is_state`, `is_district`, `is_campus`, `is_charter`

**Note:** SC uses `is_campus` (not `is_school`). `is_charter` is derived
from `charter_flag` column (`Y`/`N`). County and region columns exist
but may be NA.
