"""
Core functions wrapping scschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("scschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch South Carolina school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch South Carolina school enrollment data for multiple years.
    ...
    --------
    >>> import pyscschooldata as sc
    >>> df = sc.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
